...
def test_router_client_defaults_to_filtered_omniroute(monkeypatch):
    monkeypatch.delenv("OMNIROUTE_BASE_URL", raising=False)
    client = RouterClient()
    assert client.base_url == "http://127.0.0.1:20132/v1"


def test_router_client_does_not_emit_dummy_credentials(monkeypatch):
    monkeypatch.delenv("OMNIROUTE_API_KEY", raising=False)
    assert RouterClient().get_headers() == {"Content-Type": "application/json"}


def test_router_client_uses_explicit_api_key(monkeypatch):
